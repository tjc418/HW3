...
//	cout<<"Copying  Matrix "<<A<<"..."<<endl;	//how to do this?
...
Matrix& Matrix::operator *=(const Matrix& A) //overloading *=
{
	...
	int ncols=A.columns;
		...
		{	sum=0;
			for(int k=0; k<A.rows; k++)	//Provides Numerical Value for Mat[i,j]
			{
...
ostream& operator << (ostream& output, const Matrix& A)
{
	...
	cout<<A.GetRows()<<" x "<<A.GetColumns()<<" Matrix:"<<endl;
...
{
							//Want to add counter i++ if i<rows*columns then cout error or add zeros -T
